...
class DurationMaxPolicy(CameraAbstractPolicy):

    def apply_policy(self, config: DurationMaxConfigModel, cameras_data: typing.List[CameraData]):
            ...
            if self._match_camera(config, camera_data.camera):
                for record_segment_data in camera_data.record_segments_data:
                    ...
                    if diff >= config_value:
                        record_segment_data.policy_result = PolicyResult.DELETE
